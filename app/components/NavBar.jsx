...
const NavBar = () => {
  const { isDarkMode, setIsDarkMode, hasHydrated } = useTheme();
    ...
    >
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          ...
          className="p-2 relative w-8 h-8"
        >
          <AnimatePresence mode="wait" initial={false}>
            {isDarkMode ? (
              <motion.span
                key="sun"
                initial={{ opacity: 0, rotate: -360 * 2, scale: 0.5 }}
                animate={{ opacity: 1, rotate: 0, scale: 1 }}
                exit={{ opacity: 0, rotate: 360 * 2, scale: 0 }}
              ...
              <li key={link.path}>
                <Link
                  ...
                  className={`justify-center items-center gap-2 text-lg capitalize px-4 py-2 rounded-md transition-all hover:bg-gray-100 dark:hover:bg-gray-800 ${
                    activeLink === link.path
                      ? "border-b-4 border-darkTheme dark:border-white/50"
                      : ""
                  }`}
                  onClick={() => {
